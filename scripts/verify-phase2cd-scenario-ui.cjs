...
(async()=>{
  ...
  try{
    ...
    await page.locator('#ai-engineering-intake summary').click();
    await page.locator('#ai-eng-model').fill('SAINS-PD-CANDIDATE');
    await page.locator('#ai-eng-version').fill('1');
    await page.locator('#ai-eng-operator').fill('TEST-ENGINEER');
    await page.locator('#ai-eng-entity-id').fill('SOURCE-SEGMENT-1');
    await page.locator('#ai-eng-parameter').fill('diameter_mm');
    await page.locator('#ai-eng-value').fill('300');
    await page.locator('#ai-eng-unit').fill('mm');
    await page.locator('#ai-eng-source').fill('as-built:TEST-ONLY');
    await page.locator('#ai-eng-effective').fill('2026-09-01T00:00:00+08:00');
    await page.locator('#ai-eng-preview-manual').click();
    assert.match(await page.locator('#ai-eng-report').innerText(),/sah format 1[\s\S]*0 rekod disimpan/);
    console.log('engineering_intake_ui=PASS draft_only=true');
    ...
    await page.waitForFunction(()=>document.querySelector('#ai-test-result')?.textContent?.includes('TEST MODEL / UNCALIBRATED'),{timeout:30000});
